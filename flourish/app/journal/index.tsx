...
export default function JournalScreen() {
  ...
  const statsLine = (() => {
    ...
    const weeks = ageInfo?.ageInWeeks
      ? ` · ${ageInfo.ageInWeeks} weeks of ${activeBaby?.name ?? 'their'} story`
      : '';
    return count + weeks;
  })();
  ...
  return (
    ...
    >
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        ...
        {statsLine && <Text style={styles.stats}>{statsLine}</Text>}
      </View>

      {loading ? (
        <ActivityIndicator color={Colors.sienna} style={{ marginTop: 40 }} size="large" />
      ) : entries.length > 0 ? (
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: 'CormorantGaramond_300Light', fontSize: 30, color: Colors.ink, marginBottom: 4 },
  ...
  stats: {
    ...
    color: Colors.sienna, marginTop: 6, letterSpacing: 0.3,
  },

  entries: { padding: Spacing.xl, gap: 12 },
  entry: {
    backgroundColor: Colors.warm, borderWidth: 1, borderColor: 'rgba(196,169,160,0.2)',
    padding: Spacing.xl, position: 'relative', marginTop: 8,
  },
  tape: {
    position: 'absolute', top: -6, alignSelf: 'center',
    width: 44, height: 14, backgroundColor: 'rgba(201,169,110,0.3)', borderRadius: 1, zIndex: 1,
  },
  ...
  entryText: {
    ...
    color: Colors.inkLight, lineHeight: 26, marginBottom: 14,
  },
  tags: { flexDirection: 'row', gap: 6, flexWrap: 'wrap' },
  tag: { backgroundColor: Colors.cream, paddingVertical: 3, paddingHorizontal: 10, borderRadius: 20 },
  tagText: { fontFamily: 'DMSans_400Regular', fontSize: 9, color: Colors.inkMedium, letterSpacing: 0.5 },

  emptyState: { alignItems: 'center', paddingVertical: 60, paddingHorizontal: 40, gap: 12 },
  emptyIcon: { fontSize: 40, opacity: 0.4 },
  emptyTitle: { fontFamily: 'CormorantGaramond_300Light', fontSize: 24, color: Colors.inkMedium },
  emptySub: { fontFamily: 'DMSans_400Regular', fontSize: Typography.sizes.sm, color: Colors.inkMedium, textAlign: 'center', lineHeight: 20 },

  addCard: {
    marginHorizontal: Spacing.xl, marginBottom: Spacing.xl,
    borderWidth: 1.5, borderColor: 'rgba(196,169,160,0.35)', borderStyle: 'dashed',
    paddingVertical: 20, alignItems: 'center', gap: 6,
  },
  addIcon: { fontSize: 24 },
  addText: { fontFamily: 'CormorantGaramond_300Light_Italic', fontSize: 16, color: Colors.inkMedium },
});
